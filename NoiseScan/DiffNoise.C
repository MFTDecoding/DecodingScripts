...
//	sort(vecChip.begin(), vecChip.end(), compareInterval);
  ...
  int countz_disp[20][4];
	...
	int nb_disp = DispPix.size();
	for (int i=0; i<nb_disp; i++){
	        int Column = AddressToColumn(DispPix[i].disk,DispPix[i].tr,DispPix[i].zone,DispPix[i].col,DispPix[i].row);
	        int Row = AddressToRow(DispPix[i].disk,DispPix[i].tr,DispPix[i].zone,DispPix[i].col,DispPix[i].row);
		int index = 10*DispPix[i].half+2*DispPix[i].disk+DispPix[i].face;
		count_disp[index]++;
		countz_disp[index][DispPix[i].zone]++;

	}

	cout << '\n' << "New noisy pixels  : " << endl;
	for(int k=0;k<20;k++){   //to have the plot
	    ...
	    int f = k%2;
	    if (k < 10){
		    cout << "h0-d" << (k-f)/2 << "-f" << f << " : " << count[k] << endl;
	    }else{
		    cout << "h1-d"<< (k-f)/2-5 << "-f" << f << " : " << count[k] << endl;;
	    }
	}
	cout << '\n' << '\t' << "Detail  : " << endl;
	for(int k=0;k<20;k++){   //to have the plot
	...
	cout << '\n' << "Noisy pixels that disapeared : " << endl;
	for(int k=0;k<20;k++){   //to have the plot
	    int f = k%2;
	    if (k < 10){
		    cout << "h0-d" << (k-f)/2 << "-f" << f << " : " << count_disp[k] << endl;
	    }else{
		    cout << "h1-d"<< (k-f)/2-5 << "-f" << f << " : " << count_disp[k] << endl;;
	    }
	}
	cout << '\n' << '\t' << "Detail  : " << endl;
	for(int k=0;k<20;k++){   //to have the plot
		...
		for(int f=0; f<4; f++){
		    if (k < 10){
			    cout << '\t' << "h0-d" << (k-face)/2 << "-f" << face << "-z" << f <<" : " << countz_disp[k][f] << endl;
		    }else{
			    cout << '\t' << "h1-d"<< (k-face)/2-5 << "-f" << face << "-z" << f <<" : " << countz_disp[k][f] << endl;;
	    	}
		}
	}
	cout << '\n' << "Noisy pixels that disapeared : " << endl;
	cout << "Total new noisy pixels : " << nb_pixels << endl;
	cout << "Total noisy pixels that disappeared : " << nb_disp << endl;
	cout << "Current total nb of noisy pixels : " << tot_noise_count << endl;
